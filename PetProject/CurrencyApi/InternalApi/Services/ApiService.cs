...
public class ApiService : ICurrencyApi
{
    ...
    public async Task<Currency[]> GetAllCurrentCurrenciesAsync(string baseCurrency,
        CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            var currencies = apiDto?.Data?.Values.ToArray();
    ...
    /// <param name="cancellationToken">Токен отмены</param>
    /// <returns>Объект <see cref="CurrenciesOnDate" />, содержащий курс валюты на указанную дату</returns>
    /// <exception cref="OperationCanceledException">Операция была отменена</exception>
    public async Task<CurrenciesOnDate> GetAllCurrenciesOnDateAsync(string baseCurrency, DateOnly date,
        CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                Currencies = apiDto.Data.Values.ToArray(),
                LastUpdatedAt = apiDto.Meta.LastUpdatedAt
            };
